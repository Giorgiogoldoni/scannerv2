...
Regime visivo: 🚀 Slancio · 📈 Salita · 📉 Ribasso · ⚠️ Transizione · ↔ Laterale
"""
...
def load_universe() -> list:
    ...
    with open(path, newline='', encoding='utf-8') as f:
        for row in csv.DictReader(f):
            universe.append((
                row['ticker'], row['name'], row['categoria'],
...
def calc_atr(highs, lows, closes, period=14):
    ...
    if period-1 >= len(closes): return atr
...
def calc_sar(highs, lows, af_step=0.02, af_max=0.2):
    ...
    if len(highs) < 2: return sar, bull
    b  = True; af = af_step; ep = highs[0]; sar[0] = lows[0]; bull[0] = True
...
def calc_adx_full(highs, lows, closes, period=14):
    ...
    pdi_arr = np.full(len(closes), np.nan)
    ndi_arr = np.full(len(closes), np.nan)
    ...
    if len(dx_arr) >= period:
        adx_val = np.mean([x[0] for x in dx_arr[:period]])
        idx     = period*2
        for i in range(period, len(dx_arr)):
            adx_val = (adx_val*(period-1) + dx_arr[i][0]) / period
            if idx < len(adx):
                adx[idx]     = adx_val
                pdi_arr[idx] = dx_arr[i][1]
                ndi_arr[idx] = dx_arr[i][2]
            idx += 1
    return adx, pdi_arr, ndi_arr
...
def calc_hurst(prices_list, min_points=30):
    """
...
def classify_regime(h60, h1y, adx, pdi, ndi):
    """
    5 stati:
    🚀 Slancio      — ADX>25 + rialzista + H60>0.55
    📈 Salita       — ADX>25 + PDI>NDI
    📉 Ribasso      — ADX>25 + NDI>PDI
    ⚠️ Transizione  — ADX 20-25
    ↔ Laterale     — ADX<20
    """
    adx = adx or 0
    pdi = pdi or 0
    ...
    if adx >= 25:
        if pdi >= ndi:
            if h60 > 0.55:
                return {"code":"SLANCIO",     "label":"🚀 Slancio",      "color":"#1a7f37"}
            else:
                return {"code":"SALITA",      "label":"📈 Salita",       "color":"#2ea043"}
        else:
...
def compute_score(er, kama_above, sar_bull, ao_val, ao_rising,
    ...
    s += 10 if rvi_bull else 0
    s += min(10, max(0, (mom3m + 30) / 60 * 10))
    s += min(5,  max(0, (adx or 0) / 60 * 5))
    s += min(5,  max(0, baff * 0.5)) if baff > 0 else 0
    if rsi and rsi > 70: s *= 0.9
    if trend == "ROSSO":  s *= 0.6
    return min(100, round(s))
...
def compute_sys_rating(score, kama_above, sar_bull, ao_val,
    ...
    if categoria == "Monetario":
        return "NEUTRAL", 3, 3
    if categoria == "Short":
        conds = [not sar_bull, (ao_val or 0) < 0, not kama_above,
                 not rvi_bull, trend == "ROSSO", score >= 45]
    else:
        conds = [sar_bull, (ao_val or 0) > 0, kama_above,
                 rvi_bull, trend == "VERDE", score >= 45]
...
def classify_signal(closes, kama, sar_bull_arr, ao, er_series,
    ...
    for i in range(CONFIG["kama_n"]+5, n):
        if np.isnan(kama[i]) or np.isnan(er_series[i]): continue
        price    = closes[i]; er_val = er_series[i]
        sar_bull = bool(sar_bull_arr[i]); above = price > kama[i]
        ...
        for j in range(i, max(i-20,-1), -1):
            if np.isnan(kama[j]): break
            if closes[j] > kama[j]: baff_i += 1
            else: break
        trend_i = "VERDE" if k_rising else ("ROSSO" if kama[i] < kama[i-1] else "GRIGIO")
        ...
        if er_val < er_thresh:
            signals[i] = 'RANGING'; continue
        if not above or (not sar_bull and ao_falling3):
            signals[i] = 'SELL'; continue
        buy_base = above and sar_bull and ao_rising
        if not buy_base:
            signals[i] = 'WATCH' if above and (sar_bull or ao_rising) else ('SELL' if not above else 'RANGING')
            continue
...
def build_history(ticker, dates, signals_arr, closes):
    history = []; prev = None
    for date, sig, price in zip(dates, signals_arr, closes):
        if sig != prev and sig not in ('RANGING',):
            history.append({"date": date, "signal": sig, "price": round(float(price), 4)})
            prev = sig
    return history
...
def process_etf(etf_tuple: tuple) -> dict | None:
    ...
    try:
        ...
        adx, pdi_arr, ndi_arr = calc_adx_full(highs, lows, closes, 14)
        ...
        last_pdi      = safe_last(pdi_arr)
        last_ndi      = safe_last(ndi_arr)
        ...
        signal_date = dates[-1]; signal_bars = 0
        ...
        h60  = calc_hurst(closes_list[-60:]) if len(closes_list) >= 60 else 0.5
        h1y  = calc_hurst(closes_list)
        regime = classify_regime(h60, h1y, last_adx, last_pdi, last_ndi)
        # ── Fine regime ────────────────────────────────────────────────────────
        ...
        return {
            ...
            "tv_buy":       int(tv_buy),
            "tv_sell":      int(tv_sell),
            "er":           round(float(last_er), 4)      if last_er    else None,
            "kama":         round(float(last_kama), 4)    if last_kama  else None,
            "k_pct":        round(float(k_pct), 2),
            "kama_above":   bool(kama_above),
            "kama_trend":   trend,
            "baff":         int(baff),
            "sar_bull":     bool(last_sar_bull),
            "ao":           round(float(last_ao), 4)      if last_ao    else None,
            "ao_rising":    bool(ao_rising),
            "rvi_bull":     bool(rvi_bull),
            "rsi":          round(float(last_rsi), 1)     if last_rsi   else None,
            "adx":          round(float(last_adx), 1)     if last_adx   else None,
            "pdi":          round(float(last_pdi), 1)     if last_pdi   else None,
            "ndi":          round(float(last_ndi), 1)     if last_ndi   else None,
            "hurst_60":     h60,
            "hurst_1y":     h1y,
            "regime":       regime,
            "vol_ratio":    round(float(last_vr), 2)      if last_vr    else None,
            "bb_width":     round(float(last_bb_w), 4)    if last_bb_w  else None,
            "mom1m":        round(float(mom1m), 2),
            "mom3m":        round(float(mom3m), 2),
            "mom6m":        round(float(mom6m), 2),
            "ema20":        round(float(last_ema20), 4)   if last_ema20 else None,
            "ema50":        round(float(last_ema50), 4)   if last_ema50 else None,
            "sma200":       round(float(last_sma200), 4)  if last_sma200 else None,
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=CONFIG["max_workers"]) as executor:
        ...
        for future in as_completed(futures):
            ticker = futures[future]; done += 1
            try:
                result = future.result()
                if result:
                    results.append(result)
                    log.info(f"[{done}/{len(universe)}] {ticker} → {result['signal']} score:{result['score']} H60:{result['hurst_60']} regime:{result['regime']['code']}")
                else:
    ...
    if history_path.exists():
        try:
            ...
            log.warning(f"History non leggibile: {e}")
    ...
    reg_counts = Counter(r["regime"]["code"] for r in results if r.get("regime"))
    ...
    log.info(f"Regime distribution: {dict(reg_counts)}")
